$(document).ready(function () {
    ...
    table = $('#table-compliance').DataTable({
      ...
      initComplete: function () {
        ...
        if ($activeTab.length) {
          ...
          if (initialArea) {
            this.api().column(2).search('^' + initialArea + '$', true, false).draw();
          }
        }
      },
      scrollX: true,
      ...
      columns: [
        ...
        {
          data: 'control_description', title: 'Control Description', className: 'editableControl_description text-start all',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Control Description</span>
              <div data-field="control_description">${data || "N/A"}</div>
            </div>`;
          }
        },
        {
          data: 'compliance_area', title: 'Compliance Area', className: 'editableCompliance_area text-center all',
          render: function (data, type) {
            if (type === 'filter' || type === 'sort' || type === 'type') return data || '';
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Compliance Area</span>
              <div data-field="compliance_area">${data || "N/A"}</div>
            </div>`;
          }
        },
        {
          data: 'regulation', title: 'Regulation/Standard', className: 'editableRegulations all',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Regulation/Standard</span>
              <div data-field="regulation">${getRegulationBadges(data)}</div>
            </div>`;
          }
        },
        {
          data: 'owner', title: 'Owner', className: 'all', orderable: false,
          render: function (data) {
            let content = 'N/A';
        ...
        {
          data: 'risk_level', title: 'Risk Level', className: 'editableRisk_level all',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Risk Level</span>
              <div data-field="risk_level">${getRiskLevelBadge(data)}</div>
            </div>`;
          }
        },
        {
          data: 'control_type', title: 'Control Type', className: 'editableControl_type none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Control Type</span>
              <div data-field="control_type">${getControlTypeBadge(data)}</div>
            </div>`;
          }
        },
        {
          data: 'implementation_status', title: 'Implementation Status', className: 'editableImplementation_status none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Implementation Status</span>
              <div data-field="implementation_status">${getImplementationStatusBadge(data)}</div>
            </div>`;
          }
        },
        {
          data: 'last_assessment_date', title: 'Last Assessment Date', className: 'editableLast_assessment_date none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Last Assessment Date</span>
              <div data-field="last_assessment_date">${data || "N/A"}</div>
            </div>`;
          }
        },
        {
          data: 'next_review_date', title: 'Next Review Date', className: 'editableNext_review_date none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Next Review Date</span>
              <div data-field="next_review_date">${data || "N/A"}</div>
            </div>`;
          }
        },
        {
          data: 'status', title: 'Status', className: 'editableStatus none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Status</span>
              <div data-field="status">${getStatusBadge(data)}</div>
            </div>`;
          }
        },
        {
          data: 'audit_required', title: 'Audit Required', className: 'editableAudit_required none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Audit Required</span>
              <div data-field="audit_required">${getAuditRequiredBadge(data)}</div>
            </div>`;
          }
        },
        {
          data: 'last_audit_date', title: 'Last Audit Date', className: 'editableLast_audit_date none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Last Audit Date</span>
              <div data-field="last_audit_date">${data || "N/A"}</div>
            </div>`;
          }
        },
        {
          data: 'audit_findings', title: 'Audit Findings', className: 'editableAudit_findings none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Audit Findings</span>
              <div data-field="audit_findings">${data || "N/A"}</div>
            </div>`;
          }
        },
        {
          data: 'corrective_actions', title: 'Corrective Actions', className: 'editableCorrective_actions none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Corrective Actions</span>
              <div data-field="corrective_actions">${data || "N/A"}</div>
            </div>`;
          }
        },
        {
          data: 'action_due_date', title: 'Action Due Date', className: 'editableAction_due_date none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
              <span class="mobile-label">Action Due Date</span>
              <div data-field="action_due_date">${data || "N/A"}</div>
            </div>`;
          }
        },
        {
          data: 'responsible_person', title: 'Responsible', className: 'none', orderable: false,
          render: function (data) {
            let content = 'N/A';
            if (Array.isArray(data) && data.length > 0) {
              const maxVisible = 2;
        ...
        {
          data: 'evidence_file_reference', title: 'Evidence File Reference', className: 'none', defaultContent: 'N/A', orderable: false,
          render: function (data) {
            let content = 'N/A';
            if (data) {
        ...
        {
          data: 'notes', title: 'Notes', className: 'editableNotes none',
          defaultContent: 'N/A',
          render: function (data) {
            return `<div class="d-flex flex-column align-items-start">
        ...
        {
          data: null, title: 'Actions', className: 'all', orderable: false,
  ...
  if ($activeTab.length) {
    ...
    if (initialArea && $.fn.DataTable.isDataTable('#table-compliance')) {
      $('#table-compliance').DataTable().column(2).search('^' + initialArea + '$', true, false).draw();
    }
  }
  ...
  $('#complianceTabsHeader').on('click', '.nav-link', function (e) {
    ...
    if (area) {
      dt.column(2).search('^' + area + '$', true, false).draw();
    } else {
