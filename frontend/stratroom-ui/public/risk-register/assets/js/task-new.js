$(document).ready(function() {
    ...
    function renderTabContent(tabs) {
        ...
        tabs.forEach((tab, index) => {
            ...
            const categorySummary = tab.categorySummary;
            // console.log(tab.categorySummary);

            tabContent.append(`
                <div id="${tabId}" class="tab-pane fade ${isActive}">
                   ...
                   <i data-lucide="plus" style="width: 16px; height: 16px;"></i>
                        ...
                        <div class="card-body">
                            <table class="table table-sm table-bordered w-100" id="table-${tablePrefix}" style="--stratroom-border-color:rgba(var(--stratroom-black-rgb),0.04)">
                                ...
                                <tbody>${renderGroupedTable(groupedData, tablePrefix, categorySummary)}</tbody>
                            </table>
                        </div>
                    </div>
                </div>
    ...
    function renderGroupedTable(groupedData, parentId = "", categorySummary = []) {
        let html = "";
        let groupIndex = 0;
 ...
 // 🔹 find matching category summary
        const summary = categorySummary.find(cs =>
            cs.category === category ||
            items[0]?.category_id === cs.category_id
        );
...
const responsibleOwners = visibleOwners.map(img => `
    ...
    <li class="avatar avatar-xs pull-up" href="#attendess-list" data-bs-toggle="modal">
        <span class="avatar-initial rounded-circle" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="${remainingDependencyCount} more">+${remainingDependencyCount}</span>
            ...
            function getSummaryPriorityBadge(priority) {
                const prioritySummaryMap = {
                    "High": "label-bg-red",
                    "Medium": "label-bg-yellow",
                    "Low": "label-bg-green",
                };
                const badgeSummaryPriorityClass = prioritySummaryMap[priority] || "label-bg-gray";
                return `<span class="badge ${badgeSummaryPriorityClass} rounded-pill dropdown-toggle ms-auto">${priority}</span>`;
            }
            ...
            const summaryStatusOptions = ["Open", "In Progress", "Completed", "Pending"];
            function getSummaryStatusBadge(status) {
                ...
                const badgeSummaryStatusClass = summaryStatusMap[status] || "status-bg-gray";
                return `<span class="badge ${badgeSummaryStatusClass} rounded-pill dropdown-toggle ms-auto">${status}</span>`;
            }
            ...
            $(document).on("click", "td.editable-summaryStartdate, td.editable-summaryDuedate", function() {
 ...
 $(document).on("click", "td.editable-summaryPercent", function() {
  ...
  <i data-lucide="plus" style="width: 14px; height: 14px;"></i>
            ...
            html += `<tr class="level-0 bg-light group-header" data-id="${groupId}" role="row">             
             ...
             <div class="d-flex gap-1 align-items-center">${toggleIcon} <strong>${summary?.category_id}</strong></div>  
...
<td>${summary?.category || category}</td>
 ...
 <td class="text-center text-nowrap editable-summaryPriority">${getSummaryPriorityBadge(summary.priority)}</td>
 <td class="text-center text-nowrap editable-summaryStatus">${getSummaryStatusBadge(summary.status)}</td>
<td class="text-center text-nowrap editable-summaryStartdate">${summary.start_date || ""}</td>
                <td class="text-center text-nowrap editable-summaryDuedate">${summary.due_date || ""}</td>
                <td class="text-center text-nowrap">${summary.duration || ""}</td>
<td class="text-center text-nowrap editable-summaryPercent">
    <div class="progress-wrap green">
    <div class="progress flex-grow-1" style="--stratroom-bg-opacity:1">
      <div class="progress-bar progress-bar-striped rounded-pill" role="progressbar" style="width: 0%;" data-percent="${summary.percent_complete || ""}"></div>
    </div>
    <span class="badge"></span>
  </div></td>
  <td class="text-center text-nowrap">${summary.attachments ? 
    `<a href="/path/to/files/${summary.attachments}" download class="btn btn-sm btn-icon text-decoration-none">
    <span class="icon">
