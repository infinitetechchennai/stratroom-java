...
let riskReportInfo = {}; // Global variable to store report metadata

// Function to load data and generate PDF
function loadRiskDataAndGeneratePDF() {
    if (window.RiskApiBridge && window.RiskApiBridge.enabled) {
        window.RiskApiBridge.loadRisks()
            .then(function (rows) {
                riskData = rows || [];
                riskReportInfo = { pageTitle: 'Risk Register', period: localStorage.getItem('customperiod') || '' };
                generateRiskPDF();
            })
            .catch(function () {
                loadRiskDataFromJson();
            });
        return;
    }
    loadRiskDataFromJson();
...
function loadRiskDataFromJson() {
    $.getJSON("assets/json/risk_new.json", function (response) {
        console.log("Risk Data Loaded Successfully", response);
        if (response && response.length > 0 && response[0].data) {
            riskReportInfo = response[0];
            riskData = response[0].data;
        } else {
            riskData = response;
        }
        generateRiskPDF();
...
async function generateRiskPDF() {
    ...
    if (riskData.length > 0) {
        addCoverPage(riskReportInfo || riskData[0]);
    }
    ...
    for (let i = 0; i < riskData.length; i++) {
        ...
        let currentY = header(riskReportInfo || risk);
        ...
        if (currentY > pageHeight - 40) { pdf.addPage(); currentY = header(riskReportInfo || risk); }
        pdf.setFontSize(12);
        pdf.setFont("helvetica", "bold");
