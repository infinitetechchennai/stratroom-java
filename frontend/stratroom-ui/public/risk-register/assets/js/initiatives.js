...
$(document).ready(function () {
  ...
  const table = $('#initiativeTable').DataTable({
    data: subInitiativesData,
    autoWidth: false,
    columns: [
      {
        data: 'title',
        render: function (data, type, row) {
          let caret = `<span class="caret-icon me-2" style="visibility:hidden;"><i class="fas fa-caret-right"></i></span>`;
          if (row.activities && row.activities.length > 0) {
            caret = `<span class="caret-icon btn-expand me-2"><i class="fas fa-caret-right"></i></span>`;
          }
          return `${caret} <span style="font-weight: 700;">${escapeHtml(data)}</span>`;
        }
      },
      {
        data: 'startDate',
      ...
      {
        data: 'progress.value',
        render: (data) => renderProgress(data)
      },
      {
        data: null,
        orderable: false,
        render: () => renderActions('sub-initiative')
      }
    ],
    pageLength: 10,
    ...
    order: [], // keep original order or change as needed
    // Custom DOM to only show the table (no search, pagination, or info)
    dom: 'rt'
  });
  ...
  // Expand/collapse level 1 (Sub-Initiative)
  $('#initiativeTable tbody').on('click', '.btn-expand', function (e) {
    e.stopPropagation();
    ...
    if (row.child.isShown()) {
      ...
      const childHtml = formatChildRow(rowData);
      row.child(childHtml, 'child-row-container').show();
      tr.addClass('shown');
  ...
  // Expand/collapse level 2 (Activity)
  $('#initiativeTable tbody').on('click', '.btn-expand-act', function (e) {
    e.stopPropagation();
    const tr = $(this).closest('tr');
    const actId = tr.data('act-id');
    const taskRows = tr.closest('table').find(`.task-for-${actId}`);
  ...
  // View Toggle Logic (Table vs Gantt)
