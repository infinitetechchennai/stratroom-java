...
$(document).ready(function () {
	// Load the dynamic risks from JSON
	loadRisks();
...
function populateRiskDetails(id) {
	...
	$.ajax({
		url: "/stratroom/risk/" + id + "?loadFlag=true",
		success: function (data) {
...
function handleRiskActivitiesEvent(riskplanid, id, action) {

	$("#riskActionmonitoringForm").css('display', 'none');
	$("#riskActionmonitoringForm").trigger("reset");
	$('#risk_action_desc_popup').modal('toggle');
	$("#riskActionmonitoringForm input[name='riskaction_id']").val(id);
	$("#riskActionmonitoringForm input[name='id']").val(id);
...
function handleRiskPlanEvent(id, riskId, action) {
...
function handleRiskTreatmentEvent(id, riskId, action) {
...
function handleReviewMonitoringEvent(id, result, action) {


	$('#risk_monitoring_popup').modal('toggle');

	$("#riskMonitorForm").trigger("reset");
	$("#riskMonitor_riskid").val(result);
...
function handleRiskCauseEvent(id, riskId, action) {
...
function handleRiskConqEvent(id, conqId, action) {
	...
	if (riskcontentload == true) {
		...
		$.each(data, function (index, initiative) {
			...
			if (index == 0) {
				...
				$.ajax({
					url: "/stratroom/risk/" + initiative_load_id
						+ "?loadFlag=true",
					success: function (data) {
						$('#risk_top_details').empty();
						$('#causeconsequencebody').empty();
			...
			if (riskdeletepermission == false && riskeditpermission == false) {
				...
				if (riskeditpermission == true) {
					...
					commentsrowOptions += `<li>
                                    	<a href="#" data-toggle="modal" data-target=".risk_comments_update_popup" onclick="handleEditRiskCommentsPopUp(`+ comments.id + `,` + comments.riskId + `,'` + commentdesc + `')">Edit</a>
                                    </li>`;
				}
				if (riskdeletepermission == true) {
...
// Added risk-loader functionality
function loadRisks() {
                          ...
                          <tr>
                              <th>
                                  <p class="m-0 p-0 i-alert">
                                      ...
                                      <span class="icon" id="popoverInherentRS">
                          ...
                          <tr>
                              <th>
                                  <p class="i-alert">
                                      ...
                                      <span class="icon" id="popoverResidualRS">
                                        <i data-lucide="badge-info" style="width:12px; height: 12px;"></i>
                                        </span>
                                  </p>
                              </th>
