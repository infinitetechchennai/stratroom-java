...
if (colorPickerEl) {
  ...
  if (color === "#ffffff") {
    extraBtnCSS = `
      .btn-primary {
        --stratroom-btn-bg: #E1E1E1;
        --stratroom-btn-color: #000;
        --stratroom-btn-hover-bg:  #E1E1E1;
        --stratroom-btn-active-bg: #E1E1E1
      }
        .form-check-input:checked{
        --stratroom-primary:#000
        }
        .form-check-input:focus{
         --stratroom-primary:#000
        }
        .custom-card .card-header{
        --stratroom-card-border-color:var(--stratroom-border-color-translucent);
        }      
        .dropdown-tab > .dropdown-menu .nav-link.active{
    ...
    //   const navLinkBg = mixColors(rgbColor, "255, 255, 255", 0.84);
      ...
      styleTag.innerHTML = `
       ...
       --stratroom-btn-border-color: ${color};
        ...
        --stratroom-btn-hover-color: ${contrastText};
        --stratroom-btn-active-color: ${contrastText};
           }

           .pagination{
           --stratroom-pagination-active-color:${contrastText}; 
           }
...
function loadLanguage(lang) {
  ...
  // ✅ Detect page safely — fallback to body[data-page] OR filename in URL
  let page = document.body?.getAttribute("data-page");
  if (!page) {
    const path = window.location.pathname.split("/").pop() || "";
    page = path.replace(/\.html$/, "") || "default";
  }

  // ✅ Dynamic file path (may not always exist)
  const dynamicPath = page ? `./lang/page_${page}_${lang}.json` : null;
  ...
  // ✅ Load both static and dynamic JSON (if available)
  const fetches = [safeFetch(staticPath)];
  if (dynamicPath) fetches.push(safeFetch(dynamicPath));
  ...
  Promise.all(fetches)
    .then(([staticData, dynamicData = {}]) => {
      ...
      document.querySelectorAll("[data-translate]").forEach((el) => {
        ...
        if (el.hasAttribute("data-bs-title")) {
          ...
          if (tooltipInstance) tooltipInstance.setContent({ ".tooltip-inner": text });

        // ✅ Placeholder
        } else if ("placeholder" in el) {
          el.placeholder = text;
    ...
    .catch((err) => console.error("❌ Error loading language files:", err));
}
...
// sortable menu
    ...
    // Edit
    let editItemBtn = document.getElementById('editItem'); if (editItemBtn) editItemBtn.onclick = () => {
      if (!targetLi) return;
    ...
    let editItemForm = document.getElementById('editForm'); if (editItemForm) editItemForm.onsubmit = (e) => {
      e.preventDefault();
    ...
    // Delete
    let delItemBtn = document.getElementById('deleteItem'); if (delItemBtn) delItemBtn.onclick = () => {
    ...
    // Move Up
    let moveUpBtn = document.getElementById('moveUpItem'); if (moveUpBtn) moveUpBtn.onclick = () => {
    ...
    // Move Down
    let moveDownBtn = document.getElementById('moveDownItem'); if (moveDownBtn) moveDownBtn.onclick = () => {
