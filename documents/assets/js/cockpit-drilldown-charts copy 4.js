...
function cdcRenderType02(jsonData1) {
  try {
    ...
    function traverse(node) {
      ...
      if (node.data?.monthly) {
        ...
        Object.keys(monthlyData).forEach(month => {
          ...
          actualData.push({ x: label, y: actual });
          targetData.push({ x: label, y: target });
          gapData.push({ x: label, y: gap });
        });

        execDataMap.set(execName, {
          actualData,
          targetData,
          gapData
        });
    ...
    function renderChart(selectedExec) {
      ...
      if (selectedExec === "All") {
        execDataMap.forEach((value, execName) => {
          series.push(
            { name: `${execName} Actual`, data: value.actualData },
            { name: `${execName} Target`, data: value.targetData },
            { name: `${execName} Gap`, data: value.gapData }
          );
        });
      } else {
        ...
        if (data) {
          series = [
            { name: `${selectedExec} Actual`, data: data.actualData },
            { name: `${selectedExec} Target`, data: data.targetData },
            { name: `${selectedExec} Gap`, data: data.gapData }
          ];
        }
      }
    ...
    // On change
    document.getElementById("execSelect").addEventListener("change", (e) => {
