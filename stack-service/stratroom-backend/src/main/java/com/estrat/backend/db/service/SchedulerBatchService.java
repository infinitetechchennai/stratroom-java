...
import com.estrat.backend.db.dao.SchedulerBatchRepository;
import com.estrat.backend.db.dto.ControlPanelGeneralDTO;
import com.estrat.backend.db.resource.util.BackupUtil;
...
public class SchedulerBatchService {
    ...
    public Map<String, String> checkBackup(ControlPanelGeneralDTO controlPanelGeneralDTO) {
        ...
        if (!StringUtils.isNotEmpty((CharSequence)controlPanelGeneralDTO.getGeneralSettingValue().get("path").toString())) {
            path = String.join((CharSequence)"/", this.backupFilePath, this.orgDetailsService.findById(controlPanelGeneralDTO.getOrgId()).get().getName() + "-");
            ...
            if (userPath.contains(".sql")) {
                ...
                for (int i = 0; i <= pathArray.length - 1; ++i) {
                    ...
                    File filePath = new File(makePath = makePath == null ? String.join((CharSequence)"/", this.backupFilePath, pathArray[i]) : String.join((CharSequence)"/", makePath, pathArray[i]));
    ...
    public void saveBatch(ControlPanelGeneralDTO controlPanelGeneralDTO) {
        ...
        Optional<SchedulerBatchDetails> checkSchedulerBatchDetails = this.schedulerBatchRepository.findById(controlPanelGeneralDTO.getOrgId());
        ...
        schedulerBatchDetails = checkSchedulerBatchDetails.isPresent() ? checkSchedulerBatchDetails.get() : new SchedulerBatchDetails();
        ...
        if ((schedulerBatchDetails1 = this.schedulerBatchRepository.save(schedulerBatchDetails)).getSchedulerType() != null && schedulerBatchDetails1.getSchedulerType().contains("Now") || controlPanelGeneralDTO.getGeneralSettingValue().get("backupduration").toString().equals("Now")) {
    ...
    public void checkBatchDetails() {
        ...
        List<SchedulerBatchDetails> schedulerBatchDetailsList = this.schedulerBatchRepository.findAll();
    ...
    public void updateRestore(SchedulerBatchDetails schedulerBatchDetails) {
        Optional<ControlPanelGeneral> controlPanelGeneral = this.controlPanelGeneralRepository.findById(schedulerBatchDetails.getOrgId());
        if (controlPanelGeneral.isPresent()) {
            ControlPanelGeneralDTO controlPanelGeneralDTO = new ControlPanelGeneralDTO(controlPanelGeneral.get());
