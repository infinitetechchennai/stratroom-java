...
public class ScoreCardUtil {
    ...
    public void createKPI(FormulationKPIDTO formulationKPIDTO, ObjectivesDTO objectivesDTO) {
        ...
        Map<String, Object> stringObjectMap = formulationKPIDTO.getKpiValue();
    ...
    public void populateExpiryScoreCardNotifications() {
        try {
            List<ScoreCard> scoreCard = this.cardRepository.findAllByActive(0);
            List<ScoreCardDTO> scoreCardDTOList = scoreCard.stream().map(dbvalue -> new ScoreCardDTO(dbvalue, false)).collect(Collectors.toList());
            ...
            this.log.error("Exception occured while processing scorecard expiry notifications batch", e);
        }
    }

    public ScoreCardDTO formatDates(ScoreCardDTO scoreCardDTO) {
        Map<String, Object> stringObjectsMap = scoreCardDTO.getScoreCardValue();
        List<Date> dates = this.processDateRange(stringObjectsMap, "scorecard");
    ...
    public ScoreCardDetailsDTO formatDates(ScoreCardDetailsDTO scoreCardDTO) {
        Map<String, Object> stringObjectsMap = scoreCardDTO.getScoreCardDetailsValue();
        List<Date> dates = this.processDateRange(stringObjectsMap, "scorecardDetails");
        if (CollectionUtils.isNotEmpty((Collection)dates)) {
            scoreCardDTO.setStartDate((Date)dates.get(0));
            scoreCardDTO.setEndDate((Date)dates.get(1));
        }
