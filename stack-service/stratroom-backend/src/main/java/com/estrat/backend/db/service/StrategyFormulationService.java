 ...
 *  com.estrat.backend.db.repository.StrategyFormulationRepository
...
import com.estrat.backend.db.repository.FormulationKPIRepository;
import com.estrat.backend.db.repository.FormulationMappingRepository;
import com.estrat.backend.db.repository.FormulationObjectivesRepository;
import com.estrat.backend.db.repository.FormulationScorecardRepository;
import com.estrat.backend.db.repository.FormulationSubKPIRepository;
import com.estrat.backend.db.repository.StrategyFormulationRepository;
...
public class StrategyFormulationService {
    ...
    public StrategyFormulationDTO saveStrategyFormulation(StrategyFormulationDTO strategyFormulationDTO) {
        ...
        if (StringUtils.isNotEmpty((CharSequence)strategyFormulationDTO.getFormulationTeam())) {
            Set<FormulationUserMapping> formulationList = Arrays.asList(strategyFormulationDTO.getFormulationTeam().split(",")).stream().filter(empId -> !"0".equalsIgnoreCase((String)empId)).map(empId -> new FormulationUserMapping(strategyFormulation, empId.toString())).collect(Collectors.toSet());
    ...
    public StrategyFormulationDTO getFormulationDetails(long id) {
        Optional<StrategyFormulation> formulation = this.formulationRepository.findById(id);
    ...
    public boolean deleteFormulationDetails(long id) {
        ...
        if (formulation.isPresent()) {
            ...
            List<FormulationScoreCard> scoreCardList = strategyFormulation.getScoreCardList();
            if (CollectionUtils.isNotEmpty((Collection)scoreCardList)) {
                scoreCardList.forEach(scorecard -> {
                    if (CollectionUtils.isNotEmpty((Collection)scorecard.getObjectiveList())) {
                        List<FormulationObjectives> objList = scorecard.getObjectiveList();
    ...
    public boolean deleteFormulationObjectives(long id) {
        Optional<FormulationObjectives> formulation = this.objectivesRepository.findById(id);
        if (formulation.isPresent()) {
    ...
    public boolean deleteFormulationScorecard(long id) {
        Optional<FormulationScoreCard> formulation = this.formulationScoreCardRespository.findById(id);
        if (formulation.isPresent()) {
    ...
    public boolean deleteFormulationKPI(long id) {
        Optional<FormulationKPI> formulation = this.kpiRepository.findById(id);
        if (formulation.isPresent()) {
    ...
    public boolean deleteFormulationSubKPI(long id) {
        Optional<FormulationSubKPI> formulation = this.subKPIRepository.findById(id);
        if (formulation.isPresent()) {
    ...
    public Set<StrategyFormulationDTO> getFormulationList(String statusValue) {
        ...
        List<FormulationUserMapping> mappingList = this.formulationUserMapping.findAllByIdEmpId(profilePo);
        List<StrategyFormulation> formulationList = this.formulationRepository.findByEmpId(Long.valueOf(UserThreadLocal.get()));
        ...
        Set<StrategyFormulationDTO> mappingDTOList = CollectionUtils.isNotEmpty((Collection)mappingList) ? mappingList.stream().filter(formulation -> statusPredicate.test(formulation.getId().getFormulationId())).map(formulation -> new StrategyFormulationDTO(formulation.getId().getFormulationId(), false)).collect(Collectors.toSet()) : Collections.emptySet();
    ...
    public Set<StrategyFormulationDTO> getFormulationListWithPageId(String statusValue, String pageId) {
        List<StrategyFormulation> formulationList = this.formulationRepository.findByPageId(Long.valueOf(pageId));
        Predicate<StrategyFormulation> statusPredicate = formulation -> StringUtils.isNotEmpty((CharSequence)statusValue) ? statusValue.equalsIgnoreCase(formulation.getStatus()) : true;
        Set<StrategyFormulationDTO> formulationDTOList = CollectionUtils.isNotEmpty((Collection)formulationList) ? formulationList.stream().filter(formulation -> statusPredicate.test((StrategyFormulation)formulation)).map(formulation -> new StrategyFormulationDTO(formulation, false)).collect(Collectors.toSet()) : Collections.emptySet();
    ...
    public StrategyFormulationDTO applyFormulation(String formulaionId) {
        ...
        List<FormulationScoreCardDTO> scoreCardList = formulationDetails.getScoreCardList();
        ...
        HashMap<String, Object> scorecardDetailsValueMap = new HashMap<String, Object>();
    ...
    public void updateFormulationObject(StrategyFormulationDTO strategyFormulationDTO) {
        ...
        List<FormulationScoreCardDTO> formulationScoreCardList = formulationDetails.getScoreCardList();
        ...
        List<FormulationScoreCardDTO> toScoreCardList = toFormulationDetails.getScoreCardList();
        ...
        if (CollectionUtils.isNotEmpty((Collection)toScoreCardList)) {
            toScoreCardList.forEach(scoreCard -> {
                scoreCard.setId(0L);
                List<FormulationObjectiveDTO> objList = scoreCard.getObjectiveList();
                if (CollectionUtils.isNotEmpty((Collection)objList)) {
                    objList.forEach(obj -> {
                        obj.setId(0L);
                        List<FormulationKPIDTO> kpiList = obj.getKpiList();
                        if (CollectionUtils.isNotEmpty((Collection)kpiList)) {
                            kpiList.forEach(kpi -> {
                                kpi.setId(0L);
                                List<FormulationSubKPIDTO> subkpiList = kpi.getSubKpiList();
                                if (CollectionUtils.isNotEmpty((Collection)subkpiList)) {
                                    subkpiList.forEach(subkpi -> subkpi.setId(0L));
                                }
                            });
                        }
                    });
