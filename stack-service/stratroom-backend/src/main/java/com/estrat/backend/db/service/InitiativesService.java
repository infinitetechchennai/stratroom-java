 ...
 *  com.estrat.backend.db.dao.InitiativeDAO
...
import com.estrat.backend.db.dao.InitiativeDAO;
import com.estrat.backend.db.dao.InitiativesRepository;
...
import com.estrat.backend.db.repository.InitiativeBudgetRepository;
...
import com.estrat.backend.db.service.InitiativeTaskService;
...
public class InitiativesService {
    ...
    public void delete(Initiatives initiatives) {
        this.initiativesRepository.delete(initiatives);
    }
    ...
    public Map<String, Long> saveTracker(List<InitiativesTrackerDTO> initiatives) {
        ...
        List<InitiativesTracker> initTrackerResponse = this.initiativeTrackerRepository.findByListInitiativeIdwDate(initiativeId, endDateList, Long.valueOf(UserThreadLocal.get((String)"USER_ORG_ID")));
    ...
    public List<InitiativesDTO> checkinitiativeListByEmpId(long empId, boolean loadFlag) {
        List<com.estrat.backend.db.bean.Employee> employees = this.employeeService.getAllReporteeList(empId);
        List empList = employees.stream().map(val -> val.getEmpId()).collect(Collectors.toList());
        List<Initiatives> dbList =this.initiativesRepository.findAllByEmpIds(empList, 0);
    ...
    public List<InitiativesDTO> checkinitiativeListByDeptId() {
        ...
        List<Initiatives> dbList =this.initiativesRepository.findAllBydeptIds(deptList, 0);
    ...
    public List<InitiativesDTO> findAll(long empId, boolean flag, String pageId, String dateRange) {
        List<Initiatives> dbList =null;
        Object cacheKey = null;
        List<InitiativesDTO> kpiList = null;
        Date firstDate = null;
    ...
    public List<InitiativesDTO> initiativesListByEmpId(long empId) {
        List<InitiativesDTO> kpiList = new ArrayList<InitiativesDTO>();
        List<Initiatives> dbList =this.initiativesRepository.findAllByEmpId(Long.valueOf(empId), 0);
        if (!dbList.isEmpty() && dbList != null) {
            kpiList = dbList.stream().map(dbValue -> {
    ...
    public void populateActual(InitiativesDTO initiativesDTO) throws ParseException {
        if (Objects.nonNull(initiativesDTO.getId())) {
            ...
            List<SubInitiativesDTO> subInit = initiativesDTO.getSubInitiativeList();
            if (subInit != null) {
    ...
    public void populateTotalBudgetAndActual(InitiativesDTO initiativesDTO) throws ParseException {
        if (Objects.nonNull(initiativesDTO.getId())) {
            ...
            List<ActivitiesDTO> activityList = initiativesDTO.getActivitiesList();
            if (activityList != null) {
                for (ActivitiesDTO activitiesDTO : activityList) {
                    List<SubActivitiesDTO> subActivityList = activitiesDTO.getSubActivityList();
    ...
    public InitiativesDTO populateBudget(InitiativesDTO initiativesDTO) throws ParseException {
        if (Objects.nonNull(initiativesDTO.getId())) {
            List<InitiativesBudget> initiativesBudget_list;
            String datePeriod = UserThreadLocal.get((String)"DATE_PERIOD");
            String[] daterange = datePeriod.split("-");
            SimpleDateFormat inputFormat = new SimpleDateFormat("MM/dd/yyyy");
            Date startDate = inputFormat.parse(daterange[0]);
    ...
    public InitiativeResponseDTO deleteByInitiativeId(long initiativeId) {
        ...
        if (initiatives.isPresent()) {
            ...
            this.initiativesRepository.delete(initiative);
    ...
    public List<InitiativesDTO> initiativesList() {
        List<InitiativesDTO> kpiList = null;
        List<Initiatives> dbList =this.initiativesRepository.findAll(0);
        if (!dbList.isEmpty() && dbList != null) {
    ...
    public List<InitiativesDTO> findAll(long empId) {
        ...
        childList.add(empId);
        List<Initiatives> dbList =this.initiativesRepository.findAllByEmpIds(childList, 0);
        if (!dbList.isEmpty() && dbList != null) {
            kpiList = dbList.stream().map(dbValue -> {
                InitiativesDTO initiativesDTO = new InitiativesDTO(dbValue, false);
                if (initiativesDTO.getDepartmentId() != null) {
    ...
    public List<InitiativesDTO> findInitiativeIds(String initiativeIds) {
        ...
        List<Initiatives> dbList =this.initiativesRepository.findAllByEmpIds(new ArrayList(finalList), 0);
        if (!dbList.isEmpty() && dbList != null) {
            kpiList = dbList.stream().map(dbValue -> {
                InitiativesDTO initiativesDTO = new InitiativesDTO(dbValue, false);
                this.populateImpactDesc(initiativesDTO);
    ...
    public List<InitiativesDTO> findInitiativePageIdList(String pageIds, String dateRange) throws ParseException {
        ...
        List<Initiatives> dbList =this.initiativesRepository.findAllByPageIds(Lists.transform(Arrays.asList(pageIds.split("\\,")), (Function)Longs.stringConverter()), 0, firstDate, secondDate);
        if (!dbList.isEmpty() && dbList != null) {
            kpiList = dbList.stream().map(dbValue -> {
    ...
    public List<InitiativesDTO> findInitiativeIdList(String initiativeIds, String dateRange) throws ParseException {
        List<InitiativesDTO> kpiList = new ArrayList<InitiativesDTO>();
        ArrayList<InitiativesDTO> initiatives_out = new ArrayList<InitiativesDTO>();
        List<Initiatives> dbList =this.initiativesRepository.findByInitiativeIds(Lists.transform(Arrays.asList(initiativeIds.split("\\,")), (Function)Longs.stringConverter()), 0);
        if (!dbList.isEmpty() && dbList != null) {
            kpiList = dbList.stream().map(dbValue -> {
                InitiativesDTO initiativesDTO = new InitiativesDTO(dbValue, true);
                this.populateImpactDesc(initiativesDTO);
    ...
    public List<InitiativesDTO> findInitiativeIdListDept(String deptIds) throws ParseException {
        List<InitiativesDTO> kpiList = new ArrayList<InitiativesDTO>();
        ArrayList<InitiativesDTO> initiatives_out = new ArrayList<InitiativesDTO>();
        List<Initiatives> dbList =this.initiativesRepository.findAllBydeptIds(Lists.transform(Arrays.asList(deptIds.split("\\,")), (Function)Longs.stringConverter()), 0);
        if (!dbList.isEmpty() && dbList != null) {
            kpiList = dbList.stream().map(dbValue -> {
                InitiativesDTO initiativesDTO = new InitiativesDTO(dbValue, true);
                this.populateImpactDesc(initiativesDTO);
    ...
    public InitiativeDashBoardResponseDTO buildInitiativeDashboard(List<InitiativesDTO> initiativeList) {
        ...
        for (InitiativesDTO initiative : initiativeList) {
            ...
            if (initiative.getMileStonesList() != null) {
                System.out.println("enter in treeatment is non null");
                for (MilestonesDTO milestone : initiative.getMileStonesList()) {
                    System.out.println("treatment :: " + milestone);
                    if (milestone == null || milestone.getMileStonesValue() == null) continue;
                    String action = (String)milestone.getMileStonesValue().get("status");
            ...
            Long subinitiveCount = initiative.getSubInitiativeList() != null ? (long)initiative.getSubInitiativeList().size() : 0L;
            ...
            if (initiative.getSubInitiativeList() != null) {
                ...
                while (milestone2.hasNext()) {
                    ...
                    Long activityCount = subInitive.getActivitiesList() != null ? (long)subInitive.getActivitiesList().size() : 0L;
                    initiative.setActivityCount(activityCount);
                    totActivity = totActivity + activityCount;
                }
            }
            Long milestoneCount = initiative.getMileStonesList() != null ? (long)initiative.getMileStonesList().size() : 0L;
            initiative.setMilestoneCount(milestoneCount);
            totMilestone = totMilestone + milestoneCount;
            Long taskCount = initiative.getTaskList() != null ? (long)initiative.getTaskList().size() : 0L;
