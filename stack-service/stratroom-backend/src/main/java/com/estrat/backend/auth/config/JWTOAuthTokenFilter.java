...
public class JWTOAuthTokenFilter extends GenericFilterBean {
            ...
            if (authHeader != null && authHeader.startsWith("Bearer ")) {
                ...
                if (jwtToken.isEmpty()) {
                    ...
                    if (resolvedUserInfo != null) {
                        request.setAttribute("userInfo", resolvedUserInfo);
                ...
                LOGGER.debug("Authorization header present but not Bearer format");
                request.setAttribute("AU001", "Jwt Token missing in the request");
            } else {
                LOGGER.debug("JWT Token missing in the request");
                request.setAttribute("AU001", "Jwt Token missing in the request");
    ...
    /**
     * Compares the userInfo embedded in the JWT with the userInfo supplied via the request.
     * The token may carry the userInfo either encrypted (login flow) or already decrypted
     * (refresh-token flow), so the token claim is decrypted on demand before comparison to
     * keep both sides in the same (plain) form.
     */
