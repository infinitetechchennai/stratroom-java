 ...
 *  com.estrat.service.db.dao.BudgetDetailRepository
 ...
 *  com.estrat.service.db.repository.ControlPanelWorkflowApproverRepository
...
import com.estrat.service.db.repository.ControlPanelWorkflowApproverRepository;
...
import com.estrat.service.db.service.RiskActivitiesService;
...
import com.estrat.service.db.service.RiskPlanService;
...
public class StagingChangeService {
    ...
    public void processApproval(Long stagingChangeId, Long approverId, String decision, String comments, Long version) {
        ...
        if (decision.equals("IN PROGRESS")) {
            ...
            List childChanges = this.stagingChangesRepository.findAllByParenId(change.getChangeId());
    ...
    private boolean isFinalApproval(Long workflowId, Long approverId, Long changeId) {
        List approvers = this.workflowApproversMappingRepository.findByWorkflowId(workflowId);
    ...
    public void applyChangeToDatabase(StagingChange stagingChange, Long riskId) {
        ...
        if ("risk_details".equals(stagingChange.getTableName())) {
            ...
            if ((riskcause_list = this.riskCauseAndConsequenceRepository.findByChangeId(stagingChange.getChangeId())) != null && riskcause_list.size() > 0) {
                for (Object riskCause : riskcause_list) {
            ...
            if ((riskconq_list = this.riskConsequenceRepository.findByChangeId(stagingChange.getChangeId())) != null && riskconq_list.size() > 0) {
                for (Object riskConq : riskconq_list) {
            ...
            if ((riskplan_list = this.riskPlanRepository.findByChangeId(stagingChange.getChangeId())) != null && riskplan_list.size() > 0) {
                for (Object riskplan : riskplan_list) {
        ...
        if ("risk_event".equals(stagingChange.getTableName()) && (riskDetails_list = this.riskEventRepository.findByChangeId(stagingChange.getChangeId())) != null && riskDetails_list.size() > 0) {
            riskDetails = (RiskEvent)riskDetails_list.get(0);
            riskDetails.setStatus("APPROVED");
            this.riskEventRepository.save(riskDetails);
            if (riskDetails.getVersion() > 1L) {
                this.auditService.saveAudit("RiskEvent", riskDetails.getId().longValue(), stagingChange.getSubmittedBy().longValue(), "RiskEvent Modified");
            } else {
                this.auditService.saveAudit("RiskEvent", riskDetails.getId().longValue(), stagingChange.getSubmittedBy().longValue(), "RiskEvent Created");
            }
        }
        if ("process_enabler".equals(stagingChange.getTableName()) && (pos_list = this.processEnablerRepo.findByChangeId(stagingChange.getChangeId())) != null && pos_list.size() > 0) {
            riskDetails = (ProcessEnabler)pos_list.get(0);
            riskDetails.setStatus("APPROVED");
            this.processEnablerRepo.save(riskDetails);
            if (riskDetails.getVersion() > 1L) {
                this.auditService.saveAudit("POS", riskDetails.getId().longValue(), stagingChange.getSubmittedBy().longValue(), "POS Modified");
            } else {
                this.auditService.saveAudit("POS", riskDetails.getId().longValue(), stagingChange.getSubmittedBy().longValue(), "POS Created");
            }
        }
        if ("rpo".equals(stagingChange.getTableName()) && (rpo_list = this.rpoTableRepository.findByChangeId(stagingChange.getChangeId())) != null && rpo_list.size() > 0) {
            riskDetails = (RpoTable)rpo_list.get(0);
            riskDetails.setStatus("APPROVED");
            this.rpoTableRepository.save(riskDetails);
            if (riskDetails.getVersion() > 1L) {
                this.auditService.saveAudit("RPO", riskDetails.getId().longValue(), stagingChange.getSubmittedBy().longValue(), "RPO Modified");
            } else {
                this.auditService.saveAudit("RPO", riskDetails.getId().longValue(), stagingChange.getSubmittedBy().longValue(), "RPO Created");
            }
        }
            ...
            if (iSurvay.getImpactData() != null) {
                for (ImpactDataDto impcatData : iSurvay.getImpactData()) {
                    ...
                    if (optionalData.isPresent()) {
                        List impactData = this.impactDataRepository.findAllByImpactId(impactDetails.getId());
    ...
    public List<StagingChangeDTO> getStagingChangesForUser(String loggedInEmpId) {
        ...
        List stagingChanges = this.stagingChangesRepository.findAllBySubmittedBy(employeeId);
        ...
        List approverChanges = this.workflowHistoryRepository.findAllByApproverId(employeeId).stream().map(ApproversHistory::getChangeId).map(arg_0 -> ((StagingChangeRepository)this.stagingChangesRepository).findBychangeId(arg_0)).filter(Optional::isPresent).map(Optional::get).distinct().collect(Collectors.toList());
        List combinedChanges = Stream.concat(stagingChanges.stream(), approverChanges.stream()).distinct().collect(Collectors.toList());
    ...
    public StagingChangeDTO getStagingChangeDetails(Long changeId, String user) {
        Optional optionalChange = this.stagingChangesRepository.findById(changeId);
        StagingChange change = (StagingChange)optionalChange.orElseThrow(() -> new RuntimeException("StagingChange not found with id: " + changeId));
    ...
    public StagingChangeDTO convertToDTO(StagingChange stagingChange, String user) {
        ...
        List history = this.workflowHistoryRepository.findAllByWorkflowId(stagingChange.getWorkflowId(), stagingChange.getChangeId(), Long.valueOf(stagingChange.getVersion())).stream().map(val -> this.convertApproversHistoryToDTO(val, usersubmitted)).collect(Collectors.toList());
    ...
    public List<ApproversHistoryDTO> getApprovalHistory(Long changeId, Long user) {
        ...
        List<Object> history = new ArrayList<ApproversHistoryDTO>();
        if (change.isPresent()) {
    ...
    private String determineNextApprover(StagingChange stagingChange) {
        ...
        if (workflow != null) {
            List approvers = workflow.getApproverList().stream().sorted(Comparator.comparing(ControlPanelWorkFlowApproverMapping::getApproverOrder)).collect(Collectors.toList());
    ...
    private String updateStagingChangeStatus(StagingChange stagingChange) {
        List historyList = this.workflowHistoryRepository.findAllByWorkflowId(stagingChange.getWorkflowId(), stagingChange.getChangeId(), Long.valueOf(stagingChange.getVersion()));
    ...
    public List<StagingChangeDTO> getStagingChangeInStatus(String tableName, String status, Long empId) {
        List stagingChanges = this.stagingChangesRepository.findAllByStatusAndTable(tableName, status, empId);
    ...
    public List<StagingChangeDTO> getStagingChangeNotApproved(String tableName, Long empId) {
        List stagingChanges = this.stagingChangesRepository.findAllByNotApproved(tableName, empId);
        return stagingChanges.stream().map(change -> this.convertToDTO(change, Long.toString(empId))).collect(Collectors.toList());
    }
    ...
    public List<StagingChangeDTO> getStagingChangeWRecordId(String tableName, Long recordId) {
        List stagingChanges = this.stagingChangesRepository.getStagingChangeWRecordId(tableName, recordId);
    ...
    public List<StagingChangeDTO> getStagingChangeWChangeRecordId(Long parentrecordId, Long parentChangeid) {
        List stagingChanges = this.stagingChangesRepository.getStagingChangeWChangeRecordId(parentrecordId, parentChangeid);
        return stagingChanges.stream().map(change -> this.convertToDTO(change, Long.toString(change.getSubmittedBy()))).collect(Collectors.toList());
    }

    public List<StagingChangeDTO> getApproveVersion(String tableName, Long empId, long recordId) {
        List stagingChanges = this.stagingChangesRepository.findAllApprovedVersions(tableName, empId, Long.valueOf(recordId));
        return stagingChanges.stream().map(change -> this.convertToDTO(change, Long.toString(empId))).collect(Collectors.toList());
    }
