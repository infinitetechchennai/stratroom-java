...
public class OrgChartController {
    ...
    public java.util.Map<String, Object> retrieveOrgChart(@PathVariable(value="empId") String empId, HttpServletRequest request) {
        // Pass the upstream payload straight through as a Map so that legacy field
        // names the org-structure JS reads (ownerName / deptName / designation /
        // location / profileImage) survive untouched. Going through the Employee
        // DTO would drop those keys because Employee only declares @JsonProperty
        // mappings for name / dept / title / image -- producing the all-"N/A" UI.
        this.log.debug("Calling rest controller............");
        java.util.Map<String, Object> response = new java.util.LinkedHashMap<>();
        try {
            Object nodeList = this.chartService.fetchOrgChartDetailsRaw(empId);
            response.put("nodeList", nodeList);
        } catch (Exception e) {
            this.log.warn("Org chart backend unavailable for empId={} : {}", empId, e.getMessage());
