...
public class RoleUtil {
    ...
    public static Map<String, List<String>> filterPermissionModules(Map<String, List<String>> permissions, List<String> licenseModules) {
        ...
        // If the license module list is unavailable (license service down, transient
        // network error, missing license row, etc.), do NOT strip every permission --
        // that produces a fully-locked-out UI ("You don't have any access, pls contact
        // administrator") for an otherwise valid user. Fall back to the raw permissions.
        if (CollectionUtils.isEmpty(licenseModules)) {
            return permissions;
        }
        // Normalize the British/American spelling mismatch between the license DB
        // ("Organisation") and the permission tables / UI ("Organization") so the
        // Organization module survives the filter when the license is present.
        java.util.ArrayList<String> normalizedLicense = new java.util.ArrayList<>(licenseModules);
        if (normalizedLicense.contains("Organisation") && !normalizedLicense.contains("Organization")) {
            normalizedLicense.add("Organization");
        }
        if (normalizedLicense.contains("Organization") && !normalizedLicense.contains("Organisation")) {
            normalizedLicense.add("Organisation");
        }
        return permissions.entrySet().stream().filter(entry -> validateLicenseModules.test(entry.getKey(), normalizedLicense)).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
