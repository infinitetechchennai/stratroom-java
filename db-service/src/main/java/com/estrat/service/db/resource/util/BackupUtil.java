...
public class BackupUtil {
    ...
    public void Backupdbtosql(String source) {
        ...
        try {
            ...
            String executeCmd = "C:\\Program Files\\MySQL\\MySQL Server 8.0\\bin\\mysqldump -h " + host + " -P" + this.port + " -u" + this.username + " -p" + this.password + " --add-drop-database " + this.dbName + " --routines -r " + savePath;
    ...
    public boolean restoreDB(String source1) {
        ...
        String[] executeCmd = new String[]{"C:\\Program Files\\MySQL\\MySQL Server 8.0\\bin\\mysql", "--host=" + host, "--port=" + this.port, "--user=" + this.username, "--password=" + this.password, this.dbName, "-e", " source " + source};
        try {
            Process runtimeProcess = Runtime.getRuntime().exec(executeCmd);
            int processComplete = runtimeProcess.waitFor();
            if (processComplete != 0) {
