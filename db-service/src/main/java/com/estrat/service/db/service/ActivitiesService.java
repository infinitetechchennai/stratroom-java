 ...
 *  com.estrat.service.db.dao.ActivitiesAndTasksRepository
...
import com.estrat.service.db.dao.ActivitiesAndTasksRepository;
...
public class ActivitiesService {
    ...
    public ActivitiesDTO saveDelete(ActivitiesDetails activitiesAndTasks) {
        ...
        this.activitiesAndTasksRepository.delete((Object)activitiesAndTasks);
        return new ActivitiesDTO(activitiesAndTasks);
    }

    public List<ActivitiesDTO> findAllByInitiativesId(Long initiativesId) {
        List dbList = this.activitiesAndTasksRepository.findAllByInitiativesId(initiativesId);
    ...
    public List<ActivitiesDTO> findAllBySubInitiativesId(Long initiativesId) {
        List dbList = this.activitiesAndTasksRepository.findAllBySubInitiativesId(initiativesId);
        return dbList.stream().map(dbValue -> new ActivitiesDTO(dbValue)).collect(Collectors.toList());
    }
    ...
    public Set<ActivitiesDTO> findAll(long empId) {
        ...
        List dbList = this.activitiesMapRepository.findAllByOwnerId(Long.valueOf(empId));
    ...
    public List<ActivitiesDTO> findByEmpId(Long empId) {
        List dbList = this.activitiesAndTasksRepository.findByEmpId(empId, 0);
        return dbList.stream().map(dbValue -> new ActivitiesDTO(dbValue)).collect(Collectors.toList());
    }
}
