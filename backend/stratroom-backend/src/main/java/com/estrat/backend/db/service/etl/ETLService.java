 ...
 *  com.estrat.backend.db.repository.KPITargetDetailsRepository
 ...
 */
package com.estrat.backend.db.service.etl;
...
import com.estrat.backend.db.bean.po.KpiTargetDetailsPo;
import com.estrat.backend.db.cache.DBCache;
import com.estrat.backend.db.dao.BatchDAO;
...
import com.estrat.backend.db.repository.KPITargetDetailsRepository;
...
public class ETLService {
    ...
    public Map<String, Object> saveKpiDetails(List<KPIDetailsDTO> detailsDTOs, long batchId) {
        ...
        if (flag) {
            ...
            this.dbCache.remove(cacheKey, "dbCache");
        }
        ...
        try {
            ...
            if (orgKey == 0L && StringUtils.isNotEmpty((CharSequence)kpiDetails.getEmailAddress())) {
                ...
                if (kpiDetails.getDepartmentUniqueId() != null && !kpiDetails.getDepartmentUniqueId().isEmpty()) {
                    this.logger.error(("Department UniqueId  ::" + kpiDetails.getDepartmentUniqueId()));
                    ...
                    if (departmentDetails == null) {
                        this.logger.error(("Exception while processing ETL kpi details Department ID not found " + kpiDetails.getDepartmentUniqueId()));
                ...
                if (employeeObj == null) {
                    ...
                    this.logger.error(("Exception while processing ETL kpi details Org not found ::" + kpiDetails.getOrganizationName()));
                    return false;
                }
                if (kpiDetails.getDepartmentUniqueId() != null && !kpiDetails.getDepartmentUniqueId().isEmpty()) {
                    this.logger.error(("getDepartmentName is a dept uniqueId ::" + kpiDetails.getDepartmentUniqueId()));
                    departmentDetails = this.departmentDetailsRepository.findByDeptUniqueId(kpiDetails.getDepartmentUniqueId(), employeeObj.getOrgDetails().getOrgId(), "Active");
                    if (departmentDetails == null) {
                        kpiDetails.setCauseOfFailure("Department uniqueId  found " + kpiDetails.getDepartmentUniqueId());
                        this.logger.error(("Exception while processing ETL kpi details Department not found ::" + kpiDetails.getDepartmentUniqueId()));
                        return false;
                    }
                    kpiDetails.setDeptId(Long.valueOf(departmentDetails.getId()));
                }
            ...
            if (StringUtils.isNotEmpty((CharSequence)subkpidetails.getSubNodeKey()) || StringUtils.isNotEmpty((CharSequence)subkpidetails.getSubMeasureName())) {
                ...
                if (subDetailsPo != null) {
                    ...
                    this.kpiDetailsRepository.save(subDetailsPo);
                }
            }
            ...
            this.logger.error(("Exception while processing ETL kpi details " + ExceptionLogHelper.convertToString((Exception)e)));
        ...
        try {
            ...
            if (orgKey == 0L && StringUtils.isNotEmpty((CharSequence)kpiDetails.getEmailAddress())) {
                ...
                if (StringUtils.isEmpty((CharSequence)empId)) {
                    this.logger.error("owner not found in DB and skipping the record ");
                ...
                if (StringUtils.isNotEmpty((CharSequence)kpiDetails.getMetricCode())) {
                    ...
                    if (CollectionUtils.isEmpty(kpiList)) {
                        this.logger.error("Given KPIId not found in DB");
                    ...
                    if (CollectionUtils.isEmpty(kpiList)) {
                        this.logger.error("kpi record not found and skipping the record ");
    ...
    private boolean applyNodeKey(KPIDetailsDTO kpiDetails, Employee employee) {
        if (kpiDetails.getNodeKey() == null) {
            if (StringUtils.isEmpty((CharSequence)kpiDetails.getMeasureName()) && StringUtils.isEmpty((CharSequence)kpiDetails.getMetricCode())) {
                this.logger.error("measure name is null so skipped that record ");
            ...
            if (kpiElementDetailsPo == null) {
                this.logger.error(("wrong node key provided node key key not found in DB " + kpiDetails.getNodeKey() + " :: " + Long.parseLong(kpiDetails.getOrgKey()) + ":::" + employee.getOrgDetails().getOrgId()));
    ...
    private boolean applyNodeKeyBySubMeasure(KPIDetailsDTO kpiDetails, Employee employee) {
        if (StringUtils.isEmpty((CharSequence)kpiDetails.getSubMeasureName())) {
            this.logger.error("sub measure name is null so skipped that record ");
