 ...
 *  com.estrat.backend.db.dao.ModulePrivilegeMappingDAO
 *  com.estrat.backend.db.dao.ModulePrivilegeMappingRepo
 *  com.estrat.backend.db.dao.RolesRepository
...
import com.estrat.backend.db.bean.po.Roles;
import com.estrat.backend.db.bean.po.UserRoleManagement;
import com.estrat.backend.db.dao.ModulePrivilegeMappingDAO;
import com.estrat.backend.db.dao.ModulePrivilegeMappingRepo;
import com.estrat.backend.db.dao.RolesRepository;
...
import com.estrat.backend.db.dto.ModulePrivilegeMappingDTO;
...
import com.estrat.backend.db.repository.RoleRepository;
import com.estrat.backend.db.repository.RoleUserMappingRepository;
import com.estrat.backend.db.resource.util.RoleUtil;
...
public class RoleService {
    ...
    private void updateModuleDetailsAndPrivileges(RoleDetailsPo role, RoleDTO roleDTO) {
        List<ModuleDTO> moduleDTOS = roleDTO.getModuleList();
    ...
    public boolean removeRoleDetails(String roleId) {
        ...
        if (result.isPresent()) {
            ...
            this.roleRepository.delete(roleDetailsPo);
            return true;
        }
        return false;
    }
    ...
    public boolean removeUserRole(String empId, String roleId) {
        ...
        Optional<RoleUserMapping> result = this.roleUserMappingRepository.findById(userEmbeddedId);
        if (result.isPresent()) {
    ...
    public void removeUserRole(Long roleId) {
        List<RoleUserMapping> roleList = this.roleUserMappingRepository.findAll(roleId);
        for (RoleUserMapping role : roleList) {
            this.roleUserMappingRepository.delete(role);
        }
    }

    public void removeUserRoleByEmpId(Long empId) {
        List<RoleUserMapping> roleList = this.roleUserMappingRepository.findAllEmpId(empId);
        for (RoleUserMapping role : roleList) {
            this.roleUserMappingRepository.delete(role);
        }
    }
    ...
    public void saveUserRole(Long orgId, Long empId) {
        List<Roles> roleList = this.repository.findAll();
    ...
    public List<PrivilegeDTO> getPrivilegeList() {
        List<PrivilegeDetailsPo> privilegeDetailsPos = this.privRepository.getPrivilegeList();
    ...
    public List<ModuleDTO> getModuleList() {
        List<ModuleDetailsPo> mpDetailsPos = this.moduleRepository.getModuleList();
    ...
    public List<RoleDTO> getUserRoleList(String empId) {
        ...
        List<RoleUserMapping> userRoles = this.roleUserMappingRepository.findAllByIdEmpId(employeeProfilePo);
    ...
    public List<String> getUserList(RoleDetailsPo roleDetailsPo) {
        List<RoleUserMapping> userRoles = this.roleUserMappingRepository.findAllByIdRoleId(roleDetailsPo);
    ...
    public List<Employee> moduleRoleUserList(String moduleName) {
        ...
        List<RoleUserMapping> userRoles = this.roleUserMappingRepository.findAll(this.userRoleManagementRepository.userIdList(Long.valueOf(UserThreadLocal.get((String)"USER_ORG_ID")), 0));
    ...
    public Map<String, Set<String>> getUserPermissions(long empId) {
        ...
        for (RoleUserMapping roleUserMapping : userRoles) {
            ...
            HashSet<String> roleModules = new HashSet<String>(this.privilegeMappingDAO.getModuleList());
        ...
        this.logger.debug(("user eligible privileges " + finalPrivMap));
        return finalPrivMap;
    }

    public List<Map<String, Object>> getModuleObject(Long roleId) {
        ArrayList<Map<String, Object>> mapList = new ArrayList<Map<String, Object>>();
        List<String> module = this.privilegeMappingDAO.getModuleList();
        for (String moduleName : module) {
            ...
            List<String> moduleTags = this.privilegeMappingDAO.getModuleTagList(moduleName);
    ...
    public Map<String, Object> getParticularModulePermission(Long empId, String moduleName) {
        ...
        for (RoleUserMapping roleUserMapping : userRoles) {
            ...
            RoleDetailsPo roleDetailsPo = roleUserMapping.getId().getRoleId();
            HashSet<String> tagNameList = new HashSet<String>(this.privilegeMappingDAO.getModuleTagList(moduleName));
    ...
    public boolean updateModulePrivilege(FindDTO findDTO) {
        ...
        if (modulePrivilegeMapping.isEmpty()) {
            List<ModuleDetailsPo> moduleDTOS = this.moduleRepository.getModuleListByName(findDTO.getModuleName());
    ...
    public boolean deletePermissions(FindDTO findDTO) {
        ...
        if (findDTO.getTagName().equalsIgnoreCase("All") || findDTO.getTagName().equalsIgnoreCase("all")) {
            ...
            Optional modulePrivilegeMapping = this.modulePrivilegeMappingRepo.findBy(findDTO.getRoleId(), findDTO.getModuleName(), findDTO.getTagName());
            if (modulePrivilegeMapping.isPresent()) {
                this.logger.debug(("user eligible privileges " + modulePrivilegeMapping.get()));
                ModulePrivillageMapping privilageMapping = (ModulePrivillageMapping)modulePrivilegeMapping.get();
                if (findDTO.getPrivilegeName().equalsIgnoreCase("Create")) {
    ...
    public boolean deleteModulePrivilege(FindDTO findDTO) {
        Boolean result = false;
        List<ModulePrivillageMapping> modulePrivilegeMapping = this.modulePrivilegeMappingRepo.findBy(findDTO.getRoleId(), findDTO.getModuleName());
        if (!modulePrivilegeMapping.isEmpty()) {
    ...
    public void checkRoles() {
        List<RoleDetailsPo> roleDetailsPos = this.roleRepository.findAll();
    ...
    public void checkRoles(long empId) {
        ...
        List<Roles> roles = this.repository.findAll();
    ...
    public List<ModulePrivilegeMappingDTO> checkPermission(FindDTO findDTO) {
        List<ModulePrivillageMapping> modulePrivillageMapping;
        boolean result = false;
        EmployeeProfilePo employeeProfilePo = new EmployeeProfilePo();
    ...
    public void checkAllRoles() {
        List<RoleDetailsPo> roles = this.roleRepository.findAll();
        for (RoleDetailsPo roleDetailsPo : roles) {
            List<ModuleDTO> moduleDTOS = this.roleUtil.getModuleList(roleDetailsPo.getRoleName());
            if (moduleDTOS.isEmpty()) continue;
            for (ModuleDTO moduleDTO : moduleDTOS) {
    ...
    public List<String> getPrivilegeNameList(Long roleId, String moduleName) {
        ...
        List<Map<String, Object>> privNames = this.privilegeMappingDAO.getPrivilegesListMap(roleId, moduleName);
